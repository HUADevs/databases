CREATE OR REPLACE PROCEDURE xml AS
  ...
  BEGIN
    ...
    dbms_xmldom.setAttribute(l_nba_element, 'dataset', 'allstars');

    FOR r_div IN (SELECT te.DIVISION
                  FROM TEAMS te
                  WHERE te.YEAR = 2009 AND te.DIVISION IN ('East', 'West')
    ...
    LOOP
      ...
      FOR r_plr IN ( SELECT DISTINCT
                     ...
                     WHERE allstr.YEAR = 2009 AND allstr.CONFERENCE = r_div.DIVISION
      )
      LOOP
        ...
        , dbms_xmldom.makeNode(dbms_xmldom.createTextNode(l_domdoc, r_plr.CONFERENCE))
        );
